...
interface HorizontalDonationFormProps {
...
export default function HorizontalDonationForm({
  submitApiCall,
  onSuccess,
  title = "Make a Contribution",
  description = "Support citizen initiatives directly"
}: HorizontalDonationFormProps) {
  ...
  const nextStep = async () => {
    if (step === 1) {
      ...
      const errors = await formik.validateForm();
      if (!errors.pan && !errors.donorName) {
        setStep(3);
      }
    }
  };
    ...
    <Card className="w-full rounded-card-lg border border-border shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden bg-white">
      ...
      <div className="border-b border-border/40 bg-gradient-to-r from-bg to-white p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg md:text-xl font-display font-black text-text tracking-tight">{title}</h3>
            <p className="text-xs text-text-muted mt-0.5 font-medium">{description}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs font-bold text-text-muted select-none self-center md:self-auto">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <span className={step === 3 ? "text-text font-black" : "font-semibold"}>Complete</span>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex-1 flex flex-col justify-center py-2">
            ...
            {step === 1 && (
              <div className="space-y-6 animate-fade-in w-full max-w-2xl mx-auto">
                <div className="text-center space-y-1.5">
                  <h4 className="font-display font-black text-lg text-text">Choose contribution amount</h4>
                  <p className="text-xs text-text-muted font-medium">Select a quick preset or enter a custom sum</p>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <Input
                    ...
                    size="sm"
                    icon={<span className="text-sm font-bold text-text-light font-display">₹</span>}
            ...
            {step === 2 && (
              <div className="space-y-6 animate-fade-in w-full max-w-2xl mx-auto">
                <div className="text-center space-y-1.5">
                  <h4 className="font-display font-black text-lg text-text">Select your payment channel</h4>
                  <p className="text-xs text-text-muted font-medium">Choose between quick online portal or direct transfer</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div
                    ...
                    className={cn(
                      "flex items-center justify-between p-4 rounded-2xl border cursor-pointer select-none transition-all duration-300 bg-white",
                      paymentType === "online"
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        ...
                        paymentType === "online" ? "bg-primary text-white" : "bg-bg text-text-muted"
                      )}>
                        <Smartphone size={18} />
                      </div>
                      <div>
                        <p className="text-sm font-bold text-text">Instant Online checkout</p>
                        <p className="text-[10px] text-text-muted font-semibold mt-0.5">UPI, Cards, and Net Banking</p>
                      </div>
                    </div>
                    {paymentType === "online" && (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        "p-2.5 rounded-xl transition-colors",
                        paymentType === "manual" ? "bg-primary text-white" : "bg-bg text-text-muted"
                      )}>
                        <Building2 size={18} />
                      </div>
                      <div>
                        <p className="text-sm font-bold text-text">Direct Bank Transfer</p>
                        <p className="text-[10px] text-text-muted font-semibold mt-0.5">HDFC Account Wire Transfer</p>
                      </div>
                    </div>
                    {paymentType === "manual" && (
                      <CheckCircle2 size={18} className="text-primary shrink-0" />
            ...
            {step === 3 && (
              <div className="space-y-6 animate-fade-in w-full max-w-4xl mx-auto">
                <div className="text-center space-y-1.5">
                  <h4 className="font-display font-black text-lg text-text">Confirm & complete payment</h4>
                  <p className="text-xs text-text-muted font-medium">Review your contribution summary details below</p>
                </div>
                  ...
                  <div className="max-w-md mx-auto space-y-5">
                    ...
                    <div className="p-5 rounded-2xl bg-bg border border-border/80 space-y-4 shadow-inner">
                      <div className="flex justify-between items-center text-xs font-bold pb-2.5 border-b border-border/40">
                        <span className="text-text-muted">Total Contribution:</span>
                        <span className="text-text text-lg font-display font-black">
                          ₹{(Number(formik.values.amount) || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                        </span>
                      </div>
                    ...
                    <div className="bg-bg/40 p-4 border border-border/60 rounded-2xl flex flex-col justify-between shadow-inner">
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-x-3 gap-y-2 text-[11px] font-semibold py-2 border-y border-border/40">
                          <div>
                            <p className="text-text-light opacity-75">Account Name</p>
                            <p className="font-bold text-text mt-0.5">Global Smart Citizens</p>
                          </div>
                          <div>
                            <p className="text-text-light opacity-75">HDFC Account</p>
                            <p className="font-bold text-text mt-0.5 tracking-wider select-all">50200119596441</p>
                          </div>
                          <div className="mt-1">
                            <p className="text-text-light opacity-75">IFSC Code</p>
                            <p className="font-bold text-text mt-0.5 tracking-wider select-all">HDFC0000226</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2.5 bg-green-50 border border-green-100 rounded-xl px-3 py-2 mt-3 text-[10px] text-green-800 leading-tight font-medium">
                        ...
                        <span>Perform a standard bank transfer to HDFC then upload proof below.</span>
                      </div>
                    </div>
