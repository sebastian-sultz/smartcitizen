...
import { GitFork, GitCommit, Heart, Users, Calendar, Eye, Layers, Search } from "lucide-react";
...
export const NetworkTreeInspector = () => {
  ...
  const fetchUserDownline = async (userId: string) => {
    try {
      ...
      const res = await getUserNetwork(userId, recursive, downlinePage, downlineLimit);
      if (res && res.referrals) {
        ...
        setTotalDownlineRows(res.referrals.length < downlineLimit && downlinePage === 1 ? res.referrals.length : 100);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  }, [selectedUser, activeTab, donationsPage, donationsLimit, downlinePage, downlineLimit, recursive]);
  ...
  const columns: Header<UserResponse>[] = [
    ...
    },
    {
      label: "Phone",
      render: (u) => (
        <span className="text-[13px] font-semibold text-text-muted">{u.phone}</span>
      ),
    },
    {
      label: "Type",
    ...
    },
    {
      ...
      render: (u) => (
        <span className="text-[14px] font-bold text-text">{u.total_referrals || 0} users</span>
      ),
    },
    {
  ...
  const donationColumns: Header<PaymentAdminResponse>[] = [
    {
      label: "Date",
      render: (p) => <span className="text-[12px] text-text-muted">{formatDate(p.createdAt, "short")}</span>
    },
    {
      label: "Amount",
    ...
    {
      label: "UTR / Ref",
      render: (p) => <span className="font-mono text-text-muted text-[12px]">{p.providerReferenceId || "N/A"}</span>
    },
    {
      label: "Status",
      render: (p) => <Badge variant={p.status === "SUCCESS" ? "success" : "warning"}>{p.status}</Badge>
    }
  ];
  ...
  const downlineColumns: Header<ReferralNetworkMember>[] = [
    ...
    {
      label: "Phone",
      render: (m) => <span className="text-text-muted text-[13px]">{m.phone}</span>
    },
    {
      label: "Level",
      render: (m) => (
    ...
    },
    {
      ...
      render: (m) => <span className="font-semibold">₹{m.totalDirectDonations.toLocaleString("en-IN")}</span>
    },
    {
      label: "Network Donations",
      render: (m) => <span className="font-bold text-primary">₹{m.totalNetworkDonations.toLocaleString("en-IN")}</span>
    },
    {
      label: "Joined At",
      render: (m) => <span className="text-text-muted text-[12px]">{formatDate(m.joinedAt, "default")}</span>
    }
  ];
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col sm:flex-row gap-4 items-end justify-between">
            ...
            <div className="flex-1 max-w-md space-y-1">
              <span className="text-xs font-semibold text-text-muted">Search Coordinator (Name / Phone)</span>
              <div className="flex gap-2">
                <Input
                  ...
                  size="sm"
                />
                <Button variant="primary" onClick={triggerSearch} startIcon={<Search size={16} />} size="sm">Search</Button>
              </div>
            </div>
            ...
            <div className="w-full sm:w-[200px] space-y-1">
              ...
              <Select value={sort} onValueChange={setSort}>
                <SelectTrigger className="w-full bg-bg/40 font-semibold border-border rounded-xl" size="sm">
              ...
              <div className="flex-1 overflow-y-auto min-h-0 pr-1">
                ...
                <TabsContent value="profile" className="space-y-6">
                  <div className="flex flex-col sm:flex-row items-center gap-4 border-b border-border/40 pb-6">
                    ...
                    <div>
                      <h3 className="font-bold text-lg text-text">{selectedUser.name}</h3>
                      <p className="text-xs text-text-muted mt-0.5">ID: {formatUserSlug(selectedUser.id)}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div>
                      <span className="block text-xs font-semibold text-text-muted mb-0.5">Phone Number</span>
                      <span className="font-bold text-text">{selectedUser.phone}</span>
                    </div>
                    <div>
                      <span className="block text-xs font-semibold text-text-muted mb-0.5">Role Type</span>
                      <span className="font-bold text-text uppercase">{selectedUser.user_type}</span>
                    </div>
                    <div>
                      <span className="block text-xs font-semibold text-text-muted mb-0.5">Total Amount Contributed</span>
                      <span className="font-bold text-text">₹{(selectedUser.total_amount || 0).toLocaleString("en-IN")}</span>
                    </div>
                    <div>
                      <span className="block text-xs font-semibold text-text-muted mb-0.5">Total Transactions</span>
                      <span className="font-bold text-text">{selectedUser.total_payments || 0} payments</span>
                    </div>
                    <div>
                      <span className="block text-xs font-semibold text-text-muted mb-0.5">Direct Referrals</span>
                      <span className="font-bold text-text">{selectedUser.total_referrals || 0} users</span>
                    </div>
                    <div>
                      <span className="block text-xs font-semibold text-text-muted mb-0.5">Joined Date</span>
                      <span className="font-bold text-text">{formatDate(selectedUser.created_at, "long-in")}</span>
                    </div>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="downline" className="space-y-4">
                  <div className="flex items-center justify-between bg-bg p-3.5 rounded-2xl border border-border/40 shrink-0">
                    ...
                    <Switch checked={recursive} onCheckedChange={(val) => {
                      setRecursive(val);
                      setDownlinePage(1);
                    }} />
                  </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <Card className="bg-bg/40">
                        <CardContent className="p-5 flex items-center gap-4">
                          ...
                          <div>
                            <span className="text-xs font-semibold text-text-muted block">Direct Referrals</span>
                            <span className="text-xl font-bold text-text">{networkStats.directReferralsCount} referred</span>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card className="bg-bg/40">
                        <CardContent className="p-5 flex items-center gap-4">
                          ...
                          <div>
                            <span className="text-xs font-semibold text-text-muted block">Total Downline Network</span>
                            <span className="text-xl font-bold text-text">{networkStats.totalDownlineCount} total users</span>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card className="bg-bg/40">
                        <CardContent className="p-5 flex items-center gap-4">
                          ...
                          <div>
                            <span className="text-xs font-semibold text-text-muted block">Direct Referral Donations</span>
                            <span className="text-xl font-bold text-text">₹{networkStats.directReferralDonationAmount.toLocaleString("en-IN")}</span>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card className="bg-bg/40">
                        <CardContent className="p-5 flex items-center gap-4">
                          ...
                          <div>
                            <span className="text-xs font-semibold text-text-muted block">Total Network Fundraising</span>
                            <span className="text-xl font-bold text-text">₹{networkStats.totalNetworkDonationAmount.toLocaleString("en-IN")}</span>
                          </div>
                        </CardContent>
                      </Card>
                    </div>
                  ) : (
                    <div className="text-center py-6 text-text-muted">Failed to compute metrics.</div>
                  )}
                </TabsContent>
              </div>
