...
function PaymentStatusContent() {
  ...
  const handleDownloadReceipt = () => {
    if (receiptUrl) {
      downloadBlob(
        receiptUrl,
        `80G_Receipt_${transactionId || "donation"}.pdf`,
      );
      ...
      toast.error(
        "Tax receipt is currently unavailable. Please try again from your dashboard.",
      );
    } else {
    ...
    return (
      <Card shape="xl" className="w-full max-w-lg mx-auto text-center py-10 mt-8">
        <CardContent className="space-y-6">
          ...
          <CardTitle className="text-3xl font-display font-black text-text">
            Verification Error
          </CardTitle>
          <p className="text-text-muted text-sm max-w-md mx-auto leading-relaxed">
            {error ||
              "We could not retrieve details for this transaction. Please contact support if your account was debited."}
          </p>
          <div className="pt-4 flex flex-col sm:flex-row gap-3 justify-center">
            <Button
              onClick={() =>
                router.push(user ? "/citizen/donations" : "/donation")
              }
              variant="outline"
              size="sm"
      ...
      <CardContent className="space-y-6">
          ...
          <>
            ...
            <p className="text-text-muted text-sm max-w-md mx-auto leading-relaxed">
              ...
              <span className="font-bold text-text">
                {payment.donorName || "Anonymous Citizen"}
              </span>
              , for your generous contribution of{" "}
              <span className="font-bold text-text">
                ₹{(payment.amount / 100).toLocaleString("en-IN")}
              </span>
              . Your support empowers our local civic programs.
            </p>
          </>
        )}
        ...
        {isPending && (
          <>
            ...
            <p className="text-text-muted text-sm max-w-md mx-auto leading-relaxed">
              We are waiting for final verification from PhonePe. If money was
              deducted, the status will update to SUCCESS shortly.
            </p>
          </>
        )}
        ...
        {isFailed && (
          <>
            ...
            <p className="text-text-muted text-sm max-w-md mx-auto leading-relaxed">
              Your transaction could not be processed successfully. No amount
              was debited, or it will be refunded automatically by your bank.
            </p>
          </>
        )}
        ...
        <div className="bg-bg p-6 rounded-3xl border border-border inline-block text-left w-full mt-4 space-y-3 shadow-sm">
          <div className="flex justify-between items-center text-xs pb-2 border-b border-border/50">
            ...
            <span className="font-mono font-bold text-text">
              {payment.providerReferenceId || payment.merchantOrderId}
            </span>
          </div>
          <div className="flex justify-between items-center text-xs pb-2 border-b border-border/50">
            <span className="text-text-muted font-bold uppercase tracking-wider">
        ...
        {/* Action Row */}
        {(() => {
          const isMember = Boolean(payment.userId || user);
          return (
